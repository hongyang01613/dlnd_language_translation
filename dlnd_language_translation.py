...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    #target_text = ''.join(c for c in target_text)
    #print(source_text)
    ...
    for i in range(len(source_text)):
        temp = []
        #source_text[i] = ' '.join(source_text[i].split())
        #print(source_text[i])
        for word in source_text[i].split():
            #print(i, word)
    ...
    for i in range(len(target_text)):        
        target_text[i] += ' <EOS>'
        temp = []
        #target_text[i] = ' '.join(target_text[i].split())
        for word in target_text[i].split():
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    enc_drop = tf.contrib.rnn.DropoutWrapper(enc_lstm, output_keep_prob = keep_prob)
    enc_cell = tf.contrib.rnn.MultiRNNCell([enc_drop] * num_layers)
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
    ...
    training_logits, state, _= tf.contrib.seq2seq.dynamic_rnn_decoder(
        decoder_fn=training_decoder_fn, cell=dec_cell, inputs=dec_embed_input,
...
def decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
    ...
    #inference_logits = output_fn(inference_logits)
